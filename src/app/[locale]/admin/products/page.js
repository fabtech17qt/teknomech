    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black" style={{ color: '#0F172A' }}>Products</h2>
            <p className="text-sm mt-0.5" style={{ color: '#5A6B82' }}>Manage your MEP product catalogue.</p>
          </div>
          <a
        ...
        <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(10,35,66,0.06)' }}>
          {loading ? (
            ...
            <div className="py-16 text-center text-sm" style={{ color: '#5A6B82' }}>
              No products yet.{' '}
              <a href="/en/admin/products/new" style={{ color: '#B8893D', textDecoration: 'underline' }}>Add your first product.</a>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <th style={th} className="hidden md:table-cell">Category</th>
                  <th style={th} className="hidden lg:table-cell">Brand</th>
                  <th style={th} className="hidden md:table-cell">Status</th>
              ...
              <tbody>
                {products.map((p) => (
                  <tr key={p.id}>
                    <td style={td}>
                      <span className="font-semibold" style={{ color: '#0F172A' }}>{p.nameEn}</span>
                    </td>
                    <td style={td} className="hidden md:table-cell">
                      <span className="capitalize">{p.category?.replace(/-/g, ' ')}</span>
                    </td>
                    <td style={{ ...td, color: '#5A6B82' }} className="hidden lg:table-cell">
                      {p.brand}
                    </td>
                    <td style={td} className="hidden md:table-cell">
                      <span
